...
/*
	...
	$Header: /cvsroot/usebb/UseBB/panel.php,v 1.14 2004/05/29 09:11:54 pc_freak Exp $
...
if ( $_GET['act'] == 'login' ) {
...
} elseif ( in_array($_GET['act'], array('panel_home', 'editprofile', 'editoptions', 'editpwd', 'messenger')) ) {
	...
	if ( !$session->sess_info['user_id'] ) {
		...
		$template->parse('panel_menu', array(
		    ...
		    'panel_messenger' => ( $_GET['act'] != 'messenger' ) ? '<a href="'. $functions->make_url('panel.php', array('act' => 'messenger')).'">'.$lang['MyMessenger'].'</a>' : '<a href="'.$functions->make_url('panel.php', array('act' => 'messenger')).'"><b>'.$lang['MyMessenger'].'</b></a>',
		...
		switch ( $_GET['act'] ) {
			...
			case 'editpwd':
				...
				break;
            case 'messenger':
                require(ROOT_PATH.'sources/panel_messenger.php');
                break;
