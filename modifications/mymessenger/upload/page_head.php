...
/*
	...
	$Header: /cvsroot/usebb/UseBB/sources/page_head.php,v 1.11 2004/04/10 20:31:51 pc_freak Exp $
...
//
// messenger hack: new messages notification.
//

if($session->sess_info['user_id'] > 0) {

    $result = $db->query('SELECT * FROM '.TABLE_PREFIX.'messages WHERE message_recipient_id = "'.$session->sess_info['user_info']['id'] .'" AND message_read = "0"');

    if($db->num_rows($result) > 0) {

        $notifications = '<a style="color:red;" href="'.$functions->make_url('panel.php', array('act' => 'messenger')).'">'.$lang['MessengerNewMessages'].'</a>';
