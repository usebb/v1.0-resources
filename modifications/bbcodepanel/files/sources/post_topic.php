...
/*
	...
	$Header: /cvsroot/usebb/UseBB/sources/post_topic.php,v 1.14 2005/01/09 11:27:14 pc_freak Exp $
	...
	if ( !$forumdata['status'] && $functions->get_user_level() != 3 ) {
		...
		if ( ( $session->sess_info['user_id'] || ( !empty($_POST['user']) && preg_match(USER_PREG, $_POST['user']) && strlen($_POST['user']) <= $functions->get_config('username_max_length') ) ) && !empty($_POST['subject']) && !empty($_POST['content']) && empty($_POST['preview']) ) {
			...
			$template->parse('post_form', 'various', array(
				'form_begin' => '<form action="'.$functions->make_url('post.php', array('forum' => $_GET['forum'])).'" method="post" name="msgpost">',
